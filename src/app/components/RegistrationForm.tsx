...
export function RegistrationForm({ onSubmit, onCancel, currentPrice, isLoading = false }: RegistrationFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validateForm()) {
      onSubmit(playerName, phoneNumber);
    }
  };
    ...
    >
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-4 py-3 bg-slate-950 border-2 border-slate-800 rounded-lg focus:outline-none focus:border-green-500 transition-colors text-white placeholder-slate-500"
        />
        {errors.playerName && (
          <p className="text-red-400 text-sm mt-2">{errors.playerName}</p>
        )}
      </div>
      ...
      {/* Payment Info */}
      <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 rounded-xl p-5 md:p-6 border-2 border-green-500/30">
        <div className="flex items-start gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">
              Total Entry Fee: <span className="text-green-400 font-bold text-xl md:text-2xl">₦{currentPrice.toLocaleString()}</span>
            </p>
          </div>
        </div>
      ...
      <div className="flex items-start gap-2 text-xs text-slate-500 bg-slate-950/50 p-3 rounded-lg border border-slate-800">
        ...
        <p>Your information is secure and will only be used for tournament registration.</p>
      </div>
      ...
      <div className="flex gap-4">
        <button
          type="button"
          onClick={onCancel}
          className="flex-1 py-3 bg-slate-800 rounded-lg hover:bg-slate-700 transition-all font-semibold border border-slate-700"
        >
          Cancel
        </button>
        <button
          type="submit"
          className="flex-1 py-3 bg-gradient-to-r from-green-500 to-emerald-600 rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all font-bold shadow-lg hover:shadow-green-500/50"
        >
          Reserve Slot
        </button>
      </div>
